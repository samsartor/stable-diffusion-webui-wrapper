    ...
    try:
        ...
        if download_with == 'wget':
            ...
            for line in iter(wget.stdout.readline, ''):
                #print(line, end='')
                status(f'PROGRESS {line.strip()} {model_index+1}/{len(model_index_range)}')
            ...
            for line in iter(curl.stdout.readline, ''):
                ...
                line = line.strip()
                if line != '':
                    status(f'PROGRESS {line.split()[-1]} {model_index+1}/{len(model_index_range)}')
            ...
            with file_path.open("wb") as file:
                for data in response.iter_content(chunk_size=1024**2):
                    ...
                    print(f'{progress:.2f}')
                    status(f'PROGRESS {progress:.2f}% {model_index+1}/{len(model_index_range)}')
        else:
            assert False
